...
def init_db():
    ...
    # Миграция: проверка колонки stock
    try: c.execute("SELECT stock FROM products LIMIT 1")
    except sqlite3.OperationalError: c.execute("ALTER TABLE products ADD COLUMN stock REAL DEFAULT 999")
    
    if c.execute('SELECT COUNT(*) FROM settings').fetchone()[0] == 0:
        c.executemany('INSERT INTO settings (key, value) VALUES (?, ?)', [("secret_code", "7777"), ("pay_cash", "1"), ("pay_card_courier", "1"), ("pay_transfer", "1")])
    conn.commit()
    conn.close()
...
def checkout():
    ...
    conn.commit()
    conn.close()
    send_vk_message(d.get('vk_link'), f"🚜 Заказ #{order_id} оформлен! Сумма: {d['total']}₽. Скоро свяжемся!")
...
def get_admin_data():
    ...
    for o in conn.execute('SELECT * FROM orders ORDER BY created_at DESC').fetchall():
        ...
        orders.append(od)
    res = {"orders": orders, "products": [dict(p) for p in conn.execute('SELECT * FROM products').fetchall()]}
    conn.close()
    return jsonify(res)
